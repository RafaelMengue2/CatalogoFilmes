...
btnBuscarFilme.onclick = () => {
   if (inputBuscarFilme.value.length > 0) {
      ...
      fetch("https://www.omdbapi.com/?apikey=d1ef8736&s=" +
...
let detalhesFilme = (id) => {
   fetch("https://www.omdbapi.com/?apikey=d1ef8736&plot=full&i=" +
         id)
      .then((resp) => resp.json())
      .then((resp) => {
         let filme = new Filme(
         ...
         let mostrarFilme = document.querySelector(
            "#mostrar-filme");
         mostrarFilme.appendChild(filme.getDetalhes());

         filme.getBtnSalvar().onclick = () => {
            if (localStorage.getItem("filmesSalvos") ==
               null) {
               let listaFilmesSalvos = new Array();
               listaFilmesSalvos.push(filme);
               localStorage.setItem("filmesSalvos", JSON
                  .stringify(listaFilmesSalvos));
            } else {
               let listaFilmesSalvos = new Array();
               listaFilmesSalvos = JSON.parse(localStorage
                  .getItem("filmesSalvos"));
               listaFilmesSalvos.push(filme);
               localStorage.setItem("filmesSalvos", JSON
                  .stringify(listaFilmesSalvos));
            }
...
let listarFavoritos = () => {
   ...
   let listaFilmesSalvos = JSON.parse(localStorage.getItem(
      "filmesSalvos"));
   let filmes = new Array();
...
let excluirFilme = (filmeId) => {
   ...
   localStorage.setItem("filmesSalvos", JSON.stringify(
      listaFilmesSalvos));
